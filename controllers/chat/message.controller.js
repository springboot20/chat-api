...
export const toggleVoteToPollingVote = asyncHandler(async (req, res) => {
  ...
  const targetOption = message.polling.options.find(
    (opt) => opt._id.toString() === optionId,
  );
  if (!targetOption)
    throw new ApiError(StatusCodes.NOT_FOUND, "Option not found");

  const hasVotedForThis = targetOption.responses.some((id) =>
    id.equals(userId),
  );

  if (hasVotedForThis) {
    // 2. Un-vote (always allowed)
    targetOption.responses = targetOption.responses.filter(
      (id) => !id.equals(userId),
    );
  } else {
    // 3. Add vote
...
export const createMessage = asyncHandler(async (req, res) => {
  ...
  let mentions = [];
  try {
    mentions = rawMentions ? JSON.parse(rawMentions) : [];
  } catch (err) {
    mentions = [];
  }
      ...
      try {
        ...
        attachments.push({
          ...
          localPath: isProduction
            ? null
            : getLocalFilePath(category, attachment.filename),
  ...
  const validatedMentions = Array.isArray(mentions)
    ? mentions.filter((m) => {
        if (!m?.userId || !m?.username || typeof m.position !== "number")
          return false;
        // Confirm the mentioned user is actually a participant of this chat
        return chat.participants.some(
          (p) => p.toString() === m.userId.toString(),
        );
      })
    : [];
  ...
  if (io) {
    ...
    if (deliveredNow.length) {
      await messageModel.findByIdAndUpdate(message._id, {
        $addToSet: {
          deliveredTo: { $each: deliveredNow.map((id) => id.toString()) },
        },
        $set: { status: "delivered" },
      });
...
export const deleteChatMessage = asyncHandler(async (req, res) => {
  ...
  if (mode === "production") {
    message.attachments.forEach(
      async (file) => await deleteFileFromCloudinary(file.public_id),
    );
  } else {
...
export const reactToMessage = asyncHandler(async (req) => {
  ...
  let updatedReactions = existingReactions
    .map((reaction) => {
      ...
      return {
        ...reactionDoc,
        userIds: reactionDoc.userIds.filter(
...
export const replyToMessage = asyncHandler(async (req, res) => {
  ...
  return new ApiResponse(
    StatusCodes.OK,
    "Replied to message successfully",
    messagePayload,
  );
});
...
export const markMessagesAsSeen = asyncHandler(async (req, res) => {
  ...
  if (!isParticipant)
    throw new ApiError(StatusCodes.UNAUTHORIZED, "You are not a participant");
